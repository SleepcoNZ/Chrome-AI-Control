    ...
    if (step.description.length > 10) {
      ...
      if (screenshot.success) {
        ...
        const resp = await client.sendMessage([
          { role: 'system', content: 'You verify whether a browser action completed successfully. The system may optimize search queries (abbreviations, industry terms, etc.) so the text on the page may differ from the original request — that is FINE and counts as success if the intent is the same. Sites may also autocomplete or rewrite search terms with their own suggestions — if the results page shows relevant products/results for the intended search topic, that counts as success even if the exact wording changed. Respond with ONLY valid JSON: {"ok": true/false, "reason": "brief reason"}' },
          { role: 'user', content: `Step goal: "${step.description}"\nCurrent page: "${title}" (${url})\nDid this step complete successfully? Remember: optimized/abbreviated search queries and site autocomplete rewrites still count as success if the results are relevant to the original intent.` },
